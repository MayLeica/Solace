...
export default function GoalsPage() {
  ...
  const stats = useMemo(() => {
    ...
    return periods.map(p => {
      const filtered = goals.filter(g => g.period === p.key)
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in duration-700 pb-20 pt-6 px-4">
      ...
      <header className="px-4 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        ...
        <div className="pb-1 px-4 md:px-0">
          <div className="text-left md:text-right space-y-1">
            <span className="text-[10px] font-black tracking-[0.3em] text-[--mocha]/40 uppercase">Активных целей</span>
            <p className="text-3xl font-lora text-[--espresso] opacity-80">{goals.filter(g => !g.completed).length}</p>
          </div>
        </div>
      </header>
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-4 px-4">
        {stats.map((stat) => (
          <div key={stat.label} className="group p-6 border border-[#D4C3B5]/30 rounded-sm bg-white/50 transition-all hover:bg-white flex flex-col items-center text-center shadow-sm">
      ...
      <section className="max-w-4xl mx-auto space-y-16 pt-16 px-4">
        ...
        <div className="text-center space-y-8">
          <h2 className="font-lora text-3xl text-[--espresso] italic opacity-80">Как правильно формулировать цели и реально достигать их</h2>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-12 text-left">
            <div className="space-y-4">
              <h3 className="font-black text-[12px] uppercase tracking-[0.3em] text-[--mocha]/40">1. Почему важно уметь ставить цели</h3>
              <p className="text-[13px] text-[--espresso]/70 leading-relaxed">
                Цели — это не просто список желаний. Это навигатор, который помогает понять, куда двигаться и какие действия имеют смысл.
              </p>
            </div>
        ...
        {/* МЕТОД SMART — Дизайн как у "Affirmations" */}
        ...
        {/* 5 ШАГОВ И ГОТОВАЯ ЦЕЛЬ */}
        <div className="grid grid-cols-1 md:grid-cols-1 gap-12 pt-6">
          <div className="space-y-6">
            <h3 className="font-black text-[12px] uppercase tracking-[0.3em] text-[--mocha]/40 text-center">Как составить цель по SMART за 5 шагов</h3>
            <div className="max-w-2xl mx-auto space-y-4 text-[13px] text-[--espresso]/70 border-y border-[#D4C3B5]/10 py-8">
              <p><span className="font-black text-[--espresso] mr-2">01.</span> Назови желаемый результат. Например: «хочу больше зарабатывать».</p>
              <p><span className="font-black text-[--espresso] mr-2">02.</span> Сделай конкретным. «Хочу увеличить доход».</p>
              <p><span className="font-black text-[--espresso] mr-2">03.</span> Добавь измеримость. «Увеличить доход на 20%».</p>
              <p><span className="font-black text-[--espresso] mr-2">04.</span> Проверь достижимость. Например: повышение, новые навыки, поиск новых клиентов — это реально?</p>
              <p><span className="font-black text-[--espresso] mr-2">05.</span> Добавь срок. «...в течение 3 месяцев».</p>
            </div>
          </div>
        ...
        <div className="space-y-8 pt-10 border-t border-[#D4C3B5]/20">
          <h3 className="font-black text-[12px] uppercase tracking-[0.3em] text-[--mocha]/40 text-center">План и контроль</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10 text-[13px] text-[--espresso]/70 leading-relaxed">
            <p>
              После формулировки цели — следующий шаг: разбить её на задачи. План должен включать конкретные шаги, сроки, необходимые ресурсы и критерии прогресса.
            </p>
            <p>
              Чтобы цели достигались, важно отслеживать движение. Методы контроля: трекер привычек, еженедельный отчёт самому себе, приложение для планирования или визуальная доска.
            </p>
          </div>
          <div className="pt-8 text-center">
             <p className="font-lora text-2xl text-[--espresso] italic opacity-80">Главное правило: измеряешь — значит управляешь.</p>
          </div>
        </div>
      </section>
      ...
      <footer className="text-center pt-20">
        <div className="inline-flex items-center gap-2 px-6 py-2 bg-stone-50/50 rounded-full border border-stone-100 shadow-sm">
           <span className="text-[9px] font-bold uppercase tracking-widest text-stone-300">Solace — Твой навигатор в мире целей</span>
        </div>
      </footer>
    </div>
  )
